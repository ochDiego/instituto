...
$sentencia = $pdo->query(
    "SELECT materia_nota.id AS idNotas, parcial_1,recuperatorio_1,parcial_2,recuperatorio_2,extraordinario, periodo, alumnos.nombre as alumno, usuario_id, condicion.nombre as condicion, materias.nombre as materia FROM materia_nota 
                            ...
                            WHERE (materia_carrera.estado = 1) AND (carrera_id = 1) AND (usuario_id= $usuarioId)"
);
...
<div class="col-10">
    ...
    <h2 class="mt-4 text-center fs-1">Desarrollo de Software - Materias</h2>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <table class="table table-primary table-borderless">
                                                    ...
                                                    <tbody class="text-center">
                                                            ...
                                                            <tr class="fs-4">
                                                                <th class="<?= ($materia["parcial_1"] >= 4) ? 'text-success' : 'text-danger'; ?>"><?= (empty($materia["parcial_1"])) ? '-' : $materia["parcial_1"]; ?></th>
                                                                <td class="<?= ($materia["recuperatorio_1"] >= 4) ? 'text-success' : 'text-danger'; ?>"><b><?= (empty($materia["recuperatorio_1"])) ? '-' : $materia["recuperatorio_1"]; ?></b></td>
                                                                <td class="<?= ($materia["parcial_2"] >= 4) ? 'text-success' : 'text-danger'; ?>"><b><?= (empty($materia["parcial_2"])) ? '-' : $materia["parcial_2"]; ?></b></td>
                                                                <td class="<?= ($materia["recuperatorio_2"] >= 4) ? 'text-success' : 'text-danger'; ?>"><b><?= (empty($materia["recuperatorio_2"])) ? '-' : $materia["recuperatorio_2"]; ?></b></td>
                                                                <td class="<?= ($materia["extraordinario"] >= 4) ? 'text-success' : 'text-danger'; ?>"><b><?= (empty($materia["extraordinario"])) ? '-' : $materia["extraordinario"]; ?></b></td>
                                                            </tr>
