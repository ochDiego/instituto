<?php 
    ...
    $sentencia = $pdo->prepare("SELECT p.id,p.nombre,p.apellido,p.dni,p.telefono,p.usuario_id,u.email FROM profesores p INNER JOIN usuarios u ON p.usuario_id = u.id WHERE u.id = ? AND p.estado = 1 AND u.estado = 1 LIMIT 1");
            ...
            if(count($profesor) > 0){
                ...
                if(isset($password) AND isset($passwordConfirmacion)){
                    ...
                    if(password_verify($passwordConfirmacion,$password)){
                        //Actualizo los datos del usuario en la tabla profesores
                        $sentencia = $pdo->prepare("UPDATE profesores SET nombre = ?,apellido = ?,dni = ?,telefono = ? WHERE usuario_id = ? AND estado = 1");
                ...
                if(isset($password) AND isset($passwordConfirmacion)){
                    ...
                    if(password_verify($passwordConfirmacion,$password)){
                        //Inserto los datos del usuario en la tabla profesores
                        $sentencia = $pdo->prepare("INSERT INTO profesores(nombre,apellido,dni,telefono,usuario_id) VALUES(?,?,?,?,?)");
                        $sentencia->bindParam(1,$nombre);
                        $sentencia->bindParam(2,$apellido);
                        $sentencia->bindParam(3,$dni);
                        $sentencia->bindParam(4,$telefono);
